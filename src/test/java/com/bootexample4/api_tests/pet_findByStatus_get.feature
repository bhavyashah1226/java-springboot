...
# ********RoostGPT********
Feature: Pet Store API - Find pets by status
...
Scenario Outline: Test Find Pets By Status API with valid status
...
Scenario Outline: Test Find Pets By Status API with invalid status
  Given path '/v2/pet/findByStatus'
  And param status = '<status>'
  When method get
  Then status 400
